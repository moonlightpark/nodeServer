...
const port = 3002; // 3000:developer, 3001:main-net, 3002:sepolia

// 웹서버 start
var server = app.listen(port, function(){
    console.log(`Express Developer server has started on port ${port}`)
})
...
app.get('/senderOwnerAddress', function(req, res){ // end point 정의
	console.log('/senderOwnerAddress');
...
app.post('/ether/gasprice', function(req, res){ // end point 정의
	...
	txStore.getEtherGasPrice(web3Info.web3Instance, req.body, gasPriceStore).then(function(result){ // txStore 모듈에서 ether 전송 가스만 계산 하는 함수 호출
		console.log(result);
...
app.post('/token/gasprice', function(req, res){ // end point 정의
	...
	var token = req.body.address; // 토큰주소
	console.log(token);
	const txStore = new TxStore.txStore(req.body, constants, gasPriceStore, common, token); // txStore 생성
	txStore.getTokenGasPrice(web3Info.web3Instance, req.body, gasPriceStore).then(function(result){ // txStore 모듈에서 토큰전송 가스만 계산 하는 함수 호출
		console.log(result);
		res.status(200).send({error: false, body: result}); // 성공 return
	},function(err){
...
app.get('/ether/version/network', (req, res) => { // end point 정의
	...
	console.log('/ether/version/network : ' + web3Info.explorerUrl);
	// web3Info 에서 정보 조회
...
app.post('/multisend', (req, res) => { // end point 정의
	...
	txStore.multiSender(web3Info.web3Instance, senderContract, constants, gasPriceStore).then(function(result){ // txStore multiSender 호출
